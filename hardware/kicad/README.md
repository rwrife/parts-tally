...
This remains a PCB-layout/static-analysis deliverable. Native ERC/DRC and structural checks pass, while the installed KiCad MCP analyzer lacks `pcbnew` and no local EMC/SPICE analyzer scripts are available. Issue #7 adds a tagged KiCad 9 workflow that regenerates and validates the Gerber/drill/CPL archive; those generated files are not preferred editable source or proof of fabrication. There is no assembled prototype, continuity record, USB current measurement, RF/EMC measurement, load test, calibration, thermal test, or field test. The device remains USB/SELV-only and is not legal-for-trade or safety certified.
