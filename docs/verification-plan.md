...
| SYN-01 | Synthetic software | Feed deterministic zero, step, vibration, drift, creep, disconnect, saturation, stale, and restart fixtures to the measurement domain. | Golden outputs preserve no-count states, bounded recovery, and deterministic count/uncertainty. | Implemented in software: native golden tests assert zero, sustained steps, vibration, drift, creep, disconnect, saturation, overload, stale, restart/migration/corruption, calibration, count rounding, malformed messages, all route families, security, idempotency, sessions, export, and import. Physical characterization remains pending. |
